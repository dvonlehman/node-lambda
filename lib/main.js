...
Lambda.prototype._archivePrebuilt = function (program, archive_callback) {
  ...
  this._rsync(program, program.prebuiltDirectory, codeDirectory, false, function (err) {
    ...
    var archive = _this._whichZipProgram();
    archive = archive.bind(_this);
    archive(program, codeDirectory, archive_callback);
  });
};

Lambda.prototype._whichZipProgram = function() {
  if (process.platform === 'win32') return this._zip;

  // Test if the "zip" command exists.
  try {
    execSync('command -v zip 2>/dev/null');
  } catch (err) {
    console.log('native zip not defined, error is: ' + err.toString());
    return this._zip;
  }

  return this._nativeZip;
}
